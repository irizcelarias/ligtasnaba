...
// ✅ ping to confirm deployed
router.get("/iot/ping", (req, res) => res.json({ ok: true }));
...
router.post("/iot/report", requireAuth, requireDriver, async (req, res) => {
  ...
  try {
    ...
    return res.json({ message: "IoT status report submitted.", item: created });
  } catch (err) {
